...
{
    int i, y, x, sy, sx, s;
    params *recv_para = (params *)arg;

    for (y = recv_para->height_u; y < recv_para->height_d; ++y)
    {
        unsigned short xi[3] = {0, 0};
        xi[2] = y * y * y;
        for (x = 0; x < recv_para->width; ++x)
        {
            i = (recv_para->height - y - 1) * recv_para->width + x;
            for (sy = 0; sy < 2; ++sy)
            {
                for (sx = 0; sx < 2; ++sx)
                {
                    vec3 accumulated_color = vec3_init(0, 0, 0);
                    for (s = 0; s < recv_para->samples_per_pixel; s++)
                    {
                        /* Randomly generate a ray from camera */
                        ray camera_ray = generate_ray(&recv_para->camera, xi, recv_para->width, recv_para->height,
                                                      x, y, &recv_para->cx, &recv_para->cy, sx, sy);
                        ...
                        accumulated_color = vec3_add(accumulated_color, vec3_mulf(camera_ray_color, (1. / recv_para->samples_per_pixel)));
                    }
                    /* Add `accumulated_color` for this subpixel to the color for the image pixel, which can be imagined as 1 = 0.25 * 4 */
                    recv_para->image[i] = vec3_add(recv_para->image[i], vec3_mulf(vec3_init(clamp(accumulated_color.x), clamp(accumulated_color.y), clamp(accumulated_color.z)), .25));
                }
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < THREADS; ++i)
    {
        ...
        para[i].width = image_width;
        para[i].height = image_height;
        para[i].camera = camera;
        para[i].samples_per_pixel = samples_per_pixel;
        para[i].cx = cx;
        para[i].cy = cy;
        para[i].height_u = (image_height / 4) * i;
        para[i].height_d = (image_height / 4) * (i + 1);
        para[i].image = image;
        ret = pthread_create(&threads[i], NULL, Ray_tracing, &(para[i]));
        if (ret != 0)
        {
            printf("Create thread %d failed!", i);
            exit(1);
        }
    }
    for (i = 0; i < THREADS; ++i)

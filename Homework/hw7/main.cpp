...
int main(){
    ...
    wbuf.resize(9);
    for (int i = 0; i < 9; i++) wbuf[i] = i + 1;

    assert(rb.write_multi(9, wbuf));

    // check iterator 
    //int tmp = 101;
    std::clog << "Testing iterator operator ++: ";
    for (__detail::__iterator<int> i = rb.begin(); i != rb.end(); i+=2){
        // Testing if we can modify the numbers
        std::clog << (*i) << ' ';
    }
    std::clog << '\n';
/*
    ...
    tmp = 1;
    for (__detail::__const_iterator<int> i = rb.cbegin(); i != rb.cend(); i+=2){
        // std::clog << (*i) << ' ';
    ...
    tmp = 11;
    for (__detail::__const_iterator<int> ttt = rb2.cbegin(); ttt != rb2.end(); ttt++){
        // std::clog << *ttt << ' ';
        assert(*ttt == tmp);
        // std::clog << "| Temp" << tmp << " | ";
...
*/
    ...
    // for (std::vector<int>::iterator i = test.begin(); i != test.end(); i+=2){
    //     *i += 100;
    //     std::clog << *i << ' ';
    // }

    // for (std::vector<int>::const_iterator i = test.begin(); i != test.end(); i+=2){
    //     // *i += 100;  // fail to compile. CANNOT modify const iterator value
    //     std::clog << *i << ' ';
    // }
